...
var CheckupsStorageList = function () {
    var t = useKubevirtTranslation().t;
    var _a = useCheckupsStorageListColumns(), columns = _a[0], activeColumns = _a[1], loadedColumns = _a[2];
    var _b = useCheckupsStoragePermissions(), clusterRoleBinding = _b.clusterRoleBinding, isPermitted = _b.isPermitted, isPermittedToInstall = _b.isPermittedToInstall, loadingPermissions = _b.loading;
    var _c = useCheckupsStorageData(), configMaps = _c.configMaps, error = _c.error, jobs = _c.jobs, loading = _c.loading;
    var _d = usePagination(), onPaginationChange = _d.onPaginationChange, pagination = _d.pagination;
    var _f = useCheckupsStorageListFilters(configMaps), unfilterData = _f[0], dataFilters = _f[1], onFilterChange = _f[2], filters = _f[3];
    if (isEmpty(configMaps) && loading && !loadingPermissions && loadedColumns) {
        return (React.createElement(CheckupsStorageListEmptyState, { clusterRoleBinding: clusterRoleBinding, isPermitted: isPermitted, isPermittedToInstall: isPermittedToInstall, loadingPermissions: loadingPermissions }));
    }
    return (React.createElement(ListPageBody, null,
        ...
        React.createElement(VirtualizedTable, { EmptyMsg: function () { return (React.createElement("div", { className: "pf-u-text-align-center" }, t('No storage checkups found'))); }, rowData: {
                getJobByName: function (configMapName) {
                    return getJobByName(jobs, configMapName);
                },
            }, columns: activeColumns, data: dataFilters, loaded: loading && !loadingPermissions && loadedColumns, loadError: error, Row: CheckupsStorageListRow, unfilteredData: unfilterData })));
};
export default CheckupsStorageList;
//# sourceMappingURL=CheckupsStorageList.js.map